...
class TwistToTwistStampedAndSafety(Node):
    def __init__(self):
        super().__init__('twist_to_twist_stamped_and_safety_test_node')
        ...
        self.min_distance_sub = self.create_subscription(Float32, 'min_obstacle_distance', self.min_distance_callback, 10)
        self.controller_cmd_vel_sub = self.create_subscription(Twist, self.output_stamped_topic_, self.cmd_vel_callback, 10)
        ...
        self.distance_increase_threshold = 0.1  # meters
    ...
    def min_distance_callback(self, msg):
        self.last_min_distance = msg.data

    def cmd_vel_callback(self, msg):
        self.last_cmd_vel = msg
        self.check_safety_exit()

    def check_safety_exit(self):
        if not self.safety_stop:
            return

        # Check if the command velocity is trying to move away from obstacles
        moving_away = (self.last_cmd_vel.linear.x > 0 and self.last_min_distance < self.danger_distance) or \
                      (self.last_cmd_vel.linear.x < 0 and self.last_min_distance > self.danger_distance)

        if moving_away:
            self.get_logger().info("Attempting to exit safety stop...")
            self.safety_stop = False
    ...
    def publish_modified_twist(self):
        ...
        # Add a small fraction of the commanded velocity when in safety stop
        if self.safety_stop:
            escape_factor = 0.1  # Allow 10% of commanded velocity for escape
            twist_stamped.twist.linear.x += self.last_cmd_vel.linear.x * escape_factor
            twist_stamped.twist.angular.z += self.last_cmd_vel.angular.z * escape_factor
